...
class CoursePress_Data_Shortcode_CourseTemplate extends CoursePress_Utility {
	...
	public function get_course_join_button( $atts ) {
		...
		$atts = shortcode_atts( $defaults, $atts, 'course_join_button' );
		// Check course ID.
		...
		$now = $this->date_time_now( '00:00:00' );
		...
		$course_start_date = $this->strtotime( $course->course_start_date );
		$course_end_date = $this->strtotime( $course->course_end_date );
		$enrollment_start_date = $this->strtotime( $course->enrollment_start_date );
		$enrollment_end_date = $this->strtotime( $course->enrollment_end_date );
		...
		$enroll_course_url = add_query_arg(
			array(
				'course_id' => $course_id,
		...
		if ( is_user_logged_in() ) {
			...
			if ( 100 === $course_progress ) {
				...
				if ( is_array( $last_seen_unit ) && isset( $last_seen_unit['unit_id'] ) && isset( $last_seen_unit['page'] ) ) {
					...
					if ( ! is_wp_error( $unit ) ) {
						$course_link = $unit->get_permalink();
						$continue_learning_link = $course_link;
					}
				}
			}
		} else {
		...
		$buttons = array(
			...
			'enroll' => array(
				'label' => sanitize_text_field( $atts['enroll_text'] ),
				'attr' => array(
					'class' => $can_update_course ? 'apply-button' : 'apply-button enroll ' . $class,
					'data-link' => esc_url( $signup_url ),
			...
			'passcode' => array(
				...
				'button' => sanitize_text_field( $atts['enroll_text'] ),
				'type' => 'passcode',
			),
			'not_started' => array(
			...
			'continue' => array(
				'label' => ! $is_instructor ? sanitize_text_field( $atts['continue_learning_text'] ) : sanitize_text_field( $atts['instructor_text'] ),
				'attr' => array(
		...
		$buttons = apply_filters( 'coursepress_course_enrollment_button_options', $buttons,	$course_id );
		...
		if ( ( ! is_single() && ! is_page() ) || $list_page ) {
			...
			switch ( $buttons[ $button_option ]['type'] ) {
				...
				case 'link':
					$tag = 'a';
					if ( isset( $buttons[ $button_option ]['attr']['data-link'] ) ) {
						$url = $buttons[ $button_option ]['attr']['data-link'];
						$buttons[ $button_option ]['attr']['href'] = $url;
					} else {
						$tag = 'span';
					}
					$button = $this->create_html( $tag, $buttons[ $button_option ]['attr'], $tag_content );
					break;
				case 'passcode':
					$args = array(
						'course_id' => $course_id,
						'label_text' => esc_html( $buttons['passcode']['label'] ),
						'button_text' => esc_html( $buttons['passcode']['button'] ),
						'cookie_name' => 'cp_incorrect_passcode_' . COOKIEHASH,
					);
					$button = coursepress_render( 'views/front/passcode-form', $args, false );
					break;
			}
	 ...
	 * @return string Shortcode output.
	 */
	public function get_unit_archive_list( $atts ) {
	 ...
	 * @return string Shortcode output.
	 */
	public function get_course_featured( $atts ) {
	 ...
	 * @return bool
	 */
	public function get_course_discussion() {
		...
		if ( ! coursepress_is_true( $allow_discussion ) ) {
			return false;
		}
		$comments_args = array(
	 ...
	 * @return string
	 */
	public function get_units_dropdown( $atts ) {
		global $wp_query;
		$atts = shortcode_atts( array(
		...
		$course_id = (int) $atts['course_id'];
		$include_general = sanitize_text_field( $atts['include_general'] );
		$include_general = 'true' == $include_general ? true : false;
		$general_title = sanitize_text_field( $atts['general_title'] );
	 ...
	 * @return string|void
	 */
	public function get_course_units( $atts ) {
		$content = '';
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			if ( get_current_user_id() != $course->__get( 'post_author' ) ) {
