...
class CoursePress_Data_Shortcode_Unit extends CoursePress_Utility {
	...
	public function get_course_unit_archive_submenu( $atts ) {
		...
		$content = '<div class="submenu-main-container cp-submenu">';
		$content .= '<ul id="submenu-main" class="submenu nav-submenu">';
		...
		if ( $enrolled || $is_instructor ) {
			$content .= '<li class="submenu-item submenu-notifications ' . ( 'notifications' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course->get_notifications_url() ) . '">' . esc_html__( 'Notifications', 'cp' ) . '</a></li>';
		}

		$pages = CoursePress_Data_Course::allow_pages( $course_id );

		if ( $pages['course_discussion'] && ( $enrolled || $is_instructor ) ) {
			$content .= '<li class="submenu-item submenu-discussions ' . ( 'discussions' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course->get_discussion_url() ) . '">' . esc_html__( 'Discussions', 'cp' ) . '</a></li>';
		}

		if ( $pages['workbook'] && $enrolled ) {
			$content .= '<li class="submenu-item submenu-workbook ' . ( 'workbook' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course->get_workbook_url() ) . '">' . esc_html__( 'Workbook', 'cp' ) . '</a></li>';
		}

		if ( $pages['grades'] && $enrolled ) {
			$content .= '<li class="submenu-item submenu-grades ' . ( 'grades' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course->get_grades_url() ) . '">' . esc_html__( 'Grades', 'cp' ) . '</a></li>';
		}

		$content .= '<li class="submenu-item submenu-info"><a href="' . esc_url_raw( $course->get_permalink() ) . '">' . esc_html__( 'Course Details', 'cp' ) . '</a></li>';
		...
		if ( is_user_logged_in() && $show_link ) {
			...
			if ( $student->is_course_completed( $course_id ) ) {
				...
				$content .= '<li class="submenu-item submenu-certificate ' . ( 'certificate' == $subpage ? 'submenu-active' : '') . '">' . $certificate_link . '</li>';
			}
		}
