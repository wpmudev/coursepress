                    ...
                    <li class="<?php echo $additional_li_class;?>">
                        ...
                        <div class="unit-archive-single">
                            ...
                            <?php if (do_shortcode('[course_unit_details field="input_modules_count"]') > 0) { ?>
                                <span class="unit-archive-single-module-status"><?php if($is_unit_available){ echo do_shortcode('[course_unit_details field="student_module_responses"]'); ?> <?php _e('of', 'coursepress'); ?> <?php echo do_shortcode('[course_unit_details field="mandatory_input_modules_count"]'); ?> <?php _e('mandatory elements completed', 'coursepress'); }else{ echo __('Available', 'coursepress').' '.date(get_option('date_format'), strtotime(do_shortcode('[course_unit_details field="unit_availability"]')));} ?></span>
                            <?php } else { ?>
                                <span class="unit-archive-single-module-status"><?php if($is_unit_available){ _e('Read only'); }else{ echo __('Available', 'coursepress').' '.date(get_option('date_format'), strtotime(do_shortcode('[course_unit_details field="unit_availability"]')));}?></span>
                            <?php } ?>
                        </div>
                    </li>
                    <?php
