...
<?php } else { ?>
			...
			<div class="cp-flex cp-students">
				<div class="cp-div">
					...
					<select id="course_id" name="course_id" data-placeholder="<?php _e( 'Select a course', 'cp' ); ?>">
						...
						<?php if ( ! empty( $courses ) ) : ?>
                            ...
                            <option value="<?php echo $course->ID; ?>" <?php selected( $course->ID, $course_id ); ?>
									><?php echo $course->post_title; ?><?php echo $course->get_numeric_identifier_to_course_name( $course->ID ); ?>
							</option>
							<?php endforeach; ?>
						<?php endif; ?>
					</select>
				...
				<div class="cp-div cp-input-group-div">
					<ul class="cp-flex cp-input-group">
						<li class="cp-div-flex <?php echo ! in_array( $graded, array( 'graded', 'ungraded' ), true ) ? 'active' : ''; ?>">
							<label>
								<input type="radio" name="graded_ungraded" value="all" <?php checked( ! in_array( $graded, array( 'graded', 'ungraded' ), true ) ); ?> />
						...
						<li class="cp-div-flex <?php echo 'ungraded' === $graded ? 'active' : ''; ?>">
							<label>
		...
		<?php if ( isset( $assessments['pass_grade'] ) ) : ?>
			<ul class="cp-assessments-overview">
				<li><?php _e( 'Showing students', 'cp' ); ?>: <span class="cp-assessments-strong"><?php echo $assessments['students_count']; ?></span></li>
				<li><?php _e( 'Modules', 'cp' ); ?>: <span class="cp-assessments-strong"><?php echo $assessments['modules_count']; ?></span></li>
				<li><?php _e( 'Pass grade', 'cp' ); ?>: <span class="cp-assessments-strong"><?php echo $assessments['pass_grade']; ?>%</span></li>
				<li><?php _e( 'Grade system', 'cp' ); ?>: <span class="cp-assessments-strong"><?php echo $assessments['grade_system']; ?></span></li>
			</ul>
		<?php endif; ?>
...
<?php else : ?>
																<?php foreach ( $module['steps'] as $step_id => $step ) : ?>
																	<?php if ( ! $step->is_answerable() ) { continue; } ?>
																	<?php if ( ! $step_count ) : ?>
																	<?php endif; ?>
