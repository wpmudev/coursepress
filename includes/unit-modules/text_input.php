...
class text_input_module extends Unit_Module {
	...
	function admin_main( $data ) {
		...
		<?php echo $this->limit_attempts_element( $data ); ?>
			...
			<label class="bold-label"><?php _e( 'Content', 'coursepress' ); ?></label>
		...
		?>
			...
			<div class="answer_length">  
				<label class="bold-label"><?php _e( 'Answer Length', 'coursepress' ); ?></label>
				<input type="radio" name="<?php echo $this->name . '_answer_length[' . ( isset( $data->module_order ) ? $data->module_order : 999 ) . '][]'; ?>" value="single" <?php ?> <?php echo( isset( $data->checked_length ) && $data->checked_length == 'single' ? 'checked' : ( ! isset( $data->checked_length ) ) ? 'checked' : '' ) ?> /> <?php _e( 'Single Line', 'coursepress' ); ?><br /><br />
				<input type="radio" name="<?php echo $this->name . '_answer_length[' . ( isset( $data->module_order ) ? $data->module_order : 999 ) . '][]'; ?>" value="multi" <?php echo( isset( $data->checked_length ) && $data->checked_length == 'multi' ? 'checked' : '' ); ?> /> <?php _e( 'Multiple Lines', 'coursepress' ); ?>
			</div>
	...
	function on_create() {
		...
		$this->description = __( 'Allow students to enter a single line of text', 'coursepress' );
		$this->label       = __( 'Answer Field', 'coursepress' );
