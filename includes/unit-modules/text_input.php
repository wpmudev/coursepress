...
class text_input_module extends Unit_Module {
	...
	function admin_main( $data ) {
		...
		?>
			...
			<div class="answer_length">  
				<label class="bold-label"><?php _e( 'Answer Length', '<%= wpmudev.plugin.textdomain %>' ); ?></label>
				<input type="radio" name="<?php echo $this->name . '_answer_length[' . ( isset( $data->module_order ) ? $data->module_order : 999 ) . '][]'; ?>" value="single" <?php ?> <?php echo( isset( $data->checked_length ) && $data->checked_length == 'single' ? 'checked' : ( ! isset( $data->checked_length ) ) ? 'checked' : '' ) ?> /> <?php _e( 'Single Line', '<%= wpmudev.plugin.textdomain %>' ); ?><br /><br />
				<input type="radio" name="<?php echo $this->name . '_answer_length[' . ( isset( $data->module_order ) ? $data->module_order : 999 ) . '][]'; ?>" value="multi" <?php echo( isset( $data->checked_length ) && $data->checked_length == 'multi' ? 'checked' : '' ); ?> /> <?php _e( 'Multiple Lines', '<%= wpmudev.plugin.textdomain %>' ); ?>
			</div>
	...
	function on_create() {
		...
		$this->description = __( 'Allow students to enter a single line of text', '<%= wpmudev.plugin.textdomain %>' );
		$this->label       = __( 'Answer Field', '<%= wpmudev.plugin.textdomain %>' );
