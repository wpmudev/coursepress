...
class CP_Latest_Courses extends WP_Widget {
	...
	function CP_Latest_Courses() {
		$widget_ops = array( 'classname'   => 'cp_latest_courses_widget',
		                     'description' => __( 'Displays latest courses', '<%= wpmudev.plugin.textdomain %>' )
		);
		parent::__construct( 'CP_Latest_Courses', __( 'Latest Courses', '<%= wpmudev.plugin.textdomain %>' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Number of Courses', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
