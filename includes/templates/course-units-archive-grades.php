...
?>
<h2><?php _e( 'Course Grades', '<%= wpmudev.plugin.textdomain %>' ); ?></h2>
...
<div class="units-archive">
			...
			while ( have_posts() ) : the_post();
				...
				<li>
					...
					<?php if ( do_shortcode( '[course_unit_details field="input_modules_count"]' ) > 0 ) { ?>
						<span class="unit-archive-single-module-status"><?php echo do_shortcode( '[course_unit_details field="student_module_responses"]' ); ?> <?php _e( 'of', '<%= wpmudev.plugin.textdomain %>' ); ?> <?php echo do_shortcode( '[course_unit_details field="input_modules_count"]' ); ?> <?php _e( 'elements completed', '<%= wpmudev.plugin.textdomain %>' ); ?></span>
					<?php } else { ?>
						<span class="unit-archive-single-module-status read-only-module"><?php _e( 'Read-only', '<%= wpmudev.plugin.textdomain %>' ); ?></span>
					<?php } ?>
				</li>
				<?php
				$units ++;
	...
	<div class="total_grade"><?php echo apply_filters( 'coursepress_grade_caption', __( 'TOTAL:', '<%= wpmudev.plugin.textdomain %>' ) ); ?> <?php echo esc_html( apply_filters( 'coursepress_grade_total', ( $grades > 0 ? ( round( $grades / $units, 0 ) ) : 0 ) . '%' ) ); ?></div>
