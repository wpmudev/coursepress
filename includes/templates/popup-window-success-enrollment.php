<div class="cp_popup_inner">
	<div class="cp_popup_title cp_popup_congratulations_title"><?php _e( 'Congratulations', 'coursepress_base_td' ); ?></div>
	<?php
	global $coursepress;
	$course_id      = (int) $args['course_id'];
	$course         = new Course( $course_id );
	$dashboard_link = '<a href="' . esc_url( $coursepress->get_student_dashboard_slug( true ) ) . '">' . __( 'Dashboard', 'coursepress_base_td' ) . '</a>';
	...
	<div class="cp_popup_success_message">
		...
		<p><?php echo sprintf( __( 'You course will be available at any time in your %s', 'coursepress_base_td' ), $dashboard_link ); ?></p>
	</div>
	...
	if ( ( $course->details->course_start_date !== '' && $course->details->course_end_date !== '' ) || $course->details->open_ended_course == 'on' ) {//Course is currently active
		if ( ( strtotime( $course->details->course_start_date ) <= time() && strtotime( $course->details->course_end_date ) >= time() ) || $course->details->open_ended_course == 'on' ) {//Course is currently active
			...
			<div class="cp_popup_button_container">
				<button class="apply-button enroll-success" data-link="<?php echo esc_url( trailingslashit( get_permalink( $course_id ) ) ) . trailingslashit( $coursepress->get_units_slug() ); ?>"><?php _e( 'Start Learning Now', 'coursepress_base_td' ); ?></button>
			</div>
		<?php
		}
	} ?>
