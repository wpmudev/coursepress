...
if ( ! class_exists( 'CP_Certificate_Templates' ) ) {
	...
	class CP_Certificate_Templates {
		...
		function generate_certificate( $course_id = false, $user_id = false, $preview = false, $force_download = false ) {
			...
			$pdf->lastPage();
			$pdf->Output( ( isset( $course->details->post_name ) ? $course->details->post_name : __( 'Certificate', 'coursepress' ) ) . '.pdf', ( $force_download ? 'D' : 'I' ) );
			exit;
		}
		...
		function get_template_col_fields() {
			...
			$default_fields = array(
				array(
					...
					'field_title'       => __( 'Template Name', 'coursepress' ),
					'field_type'        => 'text',
					'field_description' => '',
				...
				array(
					...
					'field_title'       => __( 'Date', 'coursepress' ),
					'field_type'        => 'text',
					'field_description' => '',
		...
		function get_columns() {
			...
			$columns['ID'] = __( 'ID', 'coursepress' );

			foreach ( $results as $result ) {
				$columns[ $result['field_name'] ] = $result['field_title'];
			}

			$columns['edit']   = __( 'Edit', 'coursepress' );
			$columns['delete'] = __( 'Delete', 'coursepress' );
