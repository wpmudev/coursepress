	...
	class Unit_Module extends CoursePress_Object
	{
				...
				<div class='action action-draggable'>
					...
					<div class='action-body closed'>
						<?php if (!empty($this->description)) { ?>
							<p>
								<?php _e($this->description, 'coursepress_base_td'); ?>
							</p>
						<?php } ?>
		...
		public static function get_modules_front($unit_id = 0)
		{
			...
			<form name="modules_form" id="modules_form" enctype="multipart/form-data" method="post"
				  action="<?php echo trailingslashit(get_permalink($unit_id)); //strtok( $_SERVER["REQUEST_URI"], '?' ); ?>"
				...
				if (!$coursepress->is_preview($unit_id)) {
					if ($front_save) {
						if ($input_modules !== $responses) {
							?>
							<div
								class="mandatory_message"><?php _e('All questions marked with "* Mandatory" require your input.', 'coursepress_base_td'); ?></div>
							...
							<input type="submit" class="apply-button-enrolled submit-elements-data-button"
								   name="submit_modules_data_<?php echo($is_last_page ? 'done' : 'save'); ?>"
								   value="<?php echo($is_last_page ? __('Done', 'coursepress_base_td') : __('Next', 'coursepress_base_td')); ?>">
							<?php
						} else {
							?>
							<input type="submit" class="apply-button-enrolled submit-elements-data-button"
								   name="submit_modules_data_no_save_<?php echo($is_last_page ? 'done' : 'save'); ?>"
								   value="<?php echo($is_last_page ? __('Done', 'coursepress_base_td') : __('Next', 'coursepress_base_td')); ?>">
							<?php
						}
					} else {
				...
				<?php if (!isset($_GET['try'])) : ?>
					<a href="" id="save_student_progress"
					   class="save_progress"><?php _e('Save Progress & Exit', 'coursepress_base_td'); ?></a>
				<?php endif; ?>
			</form>
		...
		public static function get_module_response_comment_form($post_id)
		{
			...
			?>
			<label><?php _e('Comment', 'coursepress_base_td'); ?></label>
			<?php
		...
		function element_title_description()
		{
			?>
			<span
				class="element_title_description"><?php _e('The title is used to identify this module element and is useful for assessment.', 'coursepress_base_td'); ?></span>
			<?php
		}
		...
		function mandatory_answer_element($data)
		{
			...
			<label class="mandatory_answer">
				<input type="checkbox" name="<?php echo $this->name; ?>_mandatory_answer[]"
					   value="yes" <?php echo(isset($data->mandatory_answer) && $data->mandatory_answer == 'yes' ? 'checked' : (!isset($data->mandatory_answer)) ? 'checked' : '') ?> />
				<input type="hidden" name="<?php echo $this->name; ?>_mandatory_answer_field[]"
					   value="<?php echo((isset($data->mandatory_answer) && $data->mandatory_answer == 'yes') || !isset($data->mandatory_answer) ? 'yes' : 'no') ?>"/>
				<?php _e('Mandatory Answer', 'coursepress_base_td'); ?><br/>
				<span
					class="element_title_description"><?php _e('A response is required to continue', 'coursepress_base_td'); ?></span>
			</label>
			<?php
		}
		...
		function assessable_answer_element($data)
		{
			?>
			<label class="mandatory_answer">
				<input type="checkbox" class="assessable_checkbox" name="<?php echo $this->name; ?>_gradable_answer[]"
					   value="yes" <?php echo(isset($data->gradable_answer) && $data->gradable_answer == 'yes' ? 'checked' : (!isset($data->gradable_answer)) ? 'checked' : '') ?> />
				<input type="hidden" name="<?php echo $this->name; ?>_gradable_answer_field[]"
					   value="<?php echo((isset($data->gradable_answer) && $data->gradable_answer == 'yes') || !isset($data->gradable_answer) ? 'yes' : 'no') ?>"/>
				<?php _e('Assessable', 'coursepress_base_td'); ?><br/>
				<span
					class="element_title_description"><?php _e('The answer will be graded', 'coursepress_base_td'); ?></span>
			</label>
			<?php
		}
		...
		function placeholder_element($data)
		{
			...
			<div class="placeholder_holder">
				<label><?php _e('Placeholder Text', 'coursepress_base_td') ?><br/>
					<span
						class="element_title_description"><?php _e('Additional instructions visible in the input field as a placeholder', 'coursepress_base_td'); ?></span>
				</label>
				<input type="text" class="placeholder_text" name="<?php echo $this->name; ?>_placeholder_text[]"
					   value="<?php echo esc_attr(isset($data->placeholder_text) ? $data->placeholder_text : ''); ?>"/>
			</div>
			<?php
		}
		...
		function show_title_on_front_element($data)
		{
			...
			<label class="show_title_on_front">
				<input type="checkbox" name="<?php echo $this->name; ?>_show_title_on_front[]"
					   value="yes" <?php echo(isset($data->show_title_on_front) && $data->show_title_on_front == 'yes' ? 'checked' : (!isset($data->show_title_on_front)) ? 'checked' : '') ?> />
				<input type="hidden" name="<?php echo $this->name; ?>_show_title_field[]"
					   value="<?php echo((isset($data->show_title_on_front) && $data->show_title_on_front == 'yes') || !isset($data->show_title_on_front) ? 'yes' : 'no') ?>"/>
				<?php _e('Show Title', 'coursepress_base_td'); ?><br/>
				<span
					class="element_title_description"><?php _e('The title is displayed as a heading', 'coursepress_base_td'); ?></span>
			</label>
			<?php
		}
		...
		function minimum_grade_element($data)
		{
			...
			<label class="minimum_grade_required_label">
				<?php _e('Minimum grade required', 'coursepress_base_td'); ?>
				<input type="text" class="grade_spinner" name="<?php echo $this->name; ?>_minimum_grade_required[]"
					   value="<?php echo(isset($data->minimum_grade_required) ? $data->minimum_grade_required : 100); ?>"/><br/>
				<span
					class="element_title_description"><?php _e('Set the minimum grade (%) required to pass the task', 'coursepress_base_td'); ?></span>
			</label>
			<?php
		}
		...
		function limit_attempts_element($data)
		{
			...
			<label class="limit_attampts_label">
				<input type="checkbox" class="limit_attempts_checkbox"
					   name="<?php echo $this->name; ?>_limit_attempts[]"
					   value="yes" <?php echo(isset($data->limit_attempts) && $data->limit_attempts == 'yes' ? 'checked' : (!isset($data->limit_attempts)) ? 'checked' : '') ?> />
				<input type="hidden" name="<?php echo $this->name; ?>_limit_attempts_field[]"
					   value="<?php echo((isset($data->limit_attempts) && $data->limit_attempts == 'yes') || !isset($data->limit_attempts) ? 'yes' : 'no') ?>"/>
				<?php _e('Limit Attempts', 'coursepress_base_td'); ?>
				<input type="text" class="attempts_spinner" name="<?php echo $this->name; ?>_limit_attempts_value[]"
					   value="<?php echo(isset($data->limit_attempts_value) ? $data->limit_attempts_value : 1); ?>"/><br>
				<span
					class="element_title_description"><?php _e('Limit attempts of this task', 'coursepress_base_td'); ?></span>
			</label>
			<?php
		}
		...
		public static function mandatory_message($data)
		{
			if ('yes' == $data->mandatory_answer) {
				...
				$message = __('* Mandatory', 'coursepress_base_td');
			...
			if (isset($preview_data) && !empty($preview_data)) {
				$content = '
					<div class="module_grade">
						...
						' . esc_html__('Preview only', 'coursepress_base_td') . '
						</div>
			...
			if (
				...
				<div class="module_grade">
						...
						if ($grade_value < $minimum_grade) {
							if (($number_of_answers < $limit_attempts_value) || $limit_attempts_value == -1 || $allow_free_resubmit) {
								...
								if (empty($_GET['resubmitting'])) {
									...
									?>
									<a href="<?php echo wp_nonce_url($resubmit_url, 'resubmit_answer', 'resubmit_nonce'); ?>"
									   class="resubmit_response"><?php _e('Submit different answer', 'coursepress_base_td'); ?></a>
									<?php
								} else {
									...
									if ($module_id == $_GET['resubmitting']) {
										?>
										<a name="answer-<?php echo $previous_response; ?>"
										   href="<?php echo wp_nonce_url($resubmit_url, 'resubmit_answer', 'resubmit_nonce'); ?>"
										   class="resubmit_response cancel"><?php _e('Cancel', 'coursepress_base_td'); ?></a>
										<?php
									}
								...
								if ($attempts_remaining > 0 && !$allow_free_resubmit) {
									if ($attempts_remaining == 1) {
										_e('(1 attempt remaining)', 'coursepress_base_td');
									} else {
										printf(__('(%d attempts remaining)', 'coursepress_base_td'), $attempts_remaining);
									}
								}
							}
						}
					...
					<div class="module_grade_right">
						<?php if ($show_grade && !$allow_free_resubmit) { ?>
							<?php
							echo __('Graded: ', 'coursepress_base_td') . $grade['grade'] . '%';
							if (isset($data->minimum_grade_required) && is_numeric($data->minimum_grade_required)) {
								if ($grade['grade'] >= $data->minimum_grade_required) {
									?>
									<span class="passed_element">(<?php _e('Passed', 'coursepress_base_td'); ?>)</span>
									<?php
								} else {
									if ($attempts_remaining > 0) {
										?>
										<span
											class="failed_element">(<?php _e('Not yet passed', 'coursepress_base_td'); ?>
											)</span>
										<?php
									} else {
										?>
										<span class="failed_element">(<?php _e('Not Passed', 'coursepress_base_td'); ?>
											)</span>
										<?php
									}
								}
						...
						if ((!empty($total_correct) || 0 == $total_correct) && !empty($total_answers)) {
							printf(__('%d of %d correct', 'coursepress_base_td'), $total_correct, $total_answers);
						}
						?>
					</div>
				</div>
				...
				if ('yes' == $data->gradable_answer) {
					...
					if ((int)count($responses) > 1) {
						?>
						<div class="module_grade"><?php echo __('Grade Pending.', 'coursepress_base_td'); ?></div>
						<?php
					}
				}
			}
		...
		function time_estimation($data)
		{
			...
			?>
			<div class="module_time_estimation"><?php _e('Time Estimation (mins)', 'coursepress_base_td'); ?>
				<input type="text" name="<?php echo $this->name; ?>_time_estimation[]"
					   value="<?php echo esc_attr(isset($data->time_estimation) ? $data->time_estimation : '1:00'); ?>"/>
			</div>
			<?php
		}
		...
		{
			?>
			<a class="delete_module_link"
			   onclick="if ( deleteModule( jQuery( this ).parent().find( '.element_id' ).val() ) ) {jQuery( this ).parent().parent().remove(); update_sortable_module_indexes(); };"><i
					class="fa fa-trash-o"></i> <?php _e('Delete', 'coursepress_base_td'); ?></a>
			<?php
		}
