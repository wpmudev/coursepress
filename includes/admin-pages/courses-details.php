...
<div class="wrap nosubsub course-details cp-wrap">
	...
	<h2><?php
		if ( $course_id == '' ) {
			_e( 'New Course', 'coursepress' );
		}
		if ( $course_id != '' ) {
			_e( 'Course', 'coursepress' );
		}
	...
	<?php
	$message['ca']  = __( 'New Course added successfully!', 'coursepress' );
	$message['cu']  = __( 'Course updated successfully.', 'coursepress' );
	$message['usc'] = __( 'Unit status changed successfully', 'coursepress' );
	$message['ud']  = __( 'Unit deleted successfully', 'coursepress' );
	$message['ua']  = __( 'New Unit added successfully!', 'coursepress' );
	$message['uu']  = __( 'Unit updated successfully.', 'coursepress' );
	$message['as']  = __( 'Student added to the class successfully.', 'coursepress' );
	$message['ac']  = __( 'New class has been added successfully.', 'coursepress' );
	$message['dc']  = __( 'Selected class has been deleted successfully.', 'coursepress' );
	$message['us']  = __( 'Selected student has been withdrawed successfully from the course.', 'coursepress' );
	$message['usl'] = __( 'Selected students has been withdrawed successfully from the course.', 'coursepress' );
	$message['is']  = __( 'Invitation sent sucessfully.', 'coursepress' );
	$message['ia']  = __( 'Successfully added as instructor.', 'coursepress' );

	$error_message['wrong_email'] = __( 'Please enter valid e-mail address', 'coursepress' );
	...
	<?php
	$menus             = array();
	$menus['overview'] = __( 'Course Overview', 'coursepress' );
	$menus['units']    = __( 'Units', 'coursepress' ) . ( count( $units ) >= 1 ? ' ( ' . count( $units ) . ' )' : '' );
	$menus['students'] = __( 'Students', 'coursepress' ) . ( $students_count >= 1 ? ' ( ' . $students_count . ' )' : '' );
	...
	<h3 class="nav-tab-wrapper">
		...
		/* if ( $course_id != '' ) {
			 ...
			 ?>
			 <a class="nav-tab view-course-link" href="<?php echo get_permalink( $course_id ); ?>" target="_new"><?php _e( 'View Course', 'coursepress' ); ?></a>
			 <?php
			 }
			 } */
		?>
		...
		/* if ( $unit_id != '' ) {
		  ...
		  ?>
		  <a class="nav-tab view-course-link" href="<?php echo get_permalink( $unit_id ); ?>" target="_new"><?php _e( 'View Unit', 'coursepress' );?></a>
		  <?php
		  }
		  } */
		?>
		<?php if ( isset( $course_id ) && $course_id !== '' ) { ?>
			...
			<div class="course-state">
				...
				<span class="publish-course-message"><?php _e( 'Publish Course', 'coursepress' ); ?></span>
