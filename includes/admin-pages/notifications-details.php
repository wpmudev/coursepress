...
<div class="wrap nosubsub cp-wrap">
	...
	<h2><?php _e( 'Notification', 'coursepress' ); ?><?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_create_notification_cap' ) || current_user_can( 'coursepress_create_my_notification_cap' ) || current_user_can( 'coursepress_create_my_assigned_notification_cap' ) ) { ?>
			<a class="add-new-h2" href="<?php echo admin_url( 'admin.php?page=notifications&action=add_new' ); ?>"><?php _e( 'Add New', 'coursepress' ); ?></a><?php } ?>
	</h2>

	<?php
	$message['ca'] = __( 'New Notification added successfully!', 'coursepress' );
	$message['cu'] = __( 'Notification updated successfully.', 'coursepress' );
	?>
					...
					<div id='edit-sub' class='course-holder-wrap'>
						<div class='course-holder'>
							<div class='course-details'>
								<label for='notification_name'><?php _e( 'Notify Students in selected courses', 'coursepress' ); ?></label>

								<p><?php _e( 'Notifications are shown to end users in their Notifications menu item', 'coursepress' ); ?></p>

								<div class="full">
									<label><?php _e( 'Course', 'coursepress' ); ?></label>
									<select name="meta_course_id" class="chosen-select">
										<?php if ( current_user_can( 'coursepress_create_notification_cap' ) || current_user_can( 'coursepress_update_notification_cap' ) ) { ?>
											<option value="" <?php selected( $meta_course_id, '' ); ?>><?php _e( 'All Courses', 'coursepress' ); ?></option>
										<?php } ?>
										<?php
								...
								<label for='notification_name'><?php _e( 'Notification Title', 'coursepress' ); ?></label>
								...
								<br/><br/>
								<label for='course_name'><?php _e( 'Notification Content', 'coursepress' ); ?></label>
								<?php
								...
								<div class="buttons">
									...
									if ( current_user_can( 'manage_options' ) || ( $notification_id == 0 && current_user_can( 'coursepress_create_notification_cap' ) ) || ( $notification_id != 0 && current_user_can( 'coursepress_update_notification_cap' ) ) || ( $notification_id != 0 && current_user_can( 'coursepress_update_my_notification_cap' ) && $notification_details->post_author == get_current_user_id() ) || ( current_user_can( 'coursepress_create_my_notification_cap' ) && $available_course_options > 0 ) ) {//do not show anything
										?>
										<input type="submit" value="<?php ( $notification_id == 0 ? _e( 'Create', 'coursepress' ) : _e( 'Update', 'coursepress' ) ); ?>" class="button-primary"/>
									<?php
									} else {
										_e( 'You do not have required permissions for this action', 'coursepress' );
									}
									?>
								</div>
