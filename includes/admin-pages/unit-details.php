...
if ( !empty( $course_id ) && !CoursePress_Capabilities::can_view_course_units( $_GET[ 'course_id' ] ) ) {
	die( __( 'You do not have required permissions to access this page.', 'coursepress' ) );
}
...
if ( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'update_unit' ) {
	...
	if ( wp_verify_nonce( $_REQUEST[ '_wpnonce' ], 'unit_details_overview_' . $user_id ) ) {
		...
		/* }else{
		  die( __( 'You don\'t have right permissions for the requested action', 'coursepress' ) );
		  } */
	}
}
...
<div class='wrap mp-wrap nocoursesub unit-details cp-wrap' id='cp-unit-details-holder'>
        ...
        <ul id="sortable-units" class="mp-tabs" style="">
			...
			<?php if ( CoursePress_Capabilities::can_create_course_unit( $course_id ) ) { ?>
				<li class="mp-tab <?php echo (!isset( $_GET[ 'unit_id' ] ) ? 'active' : '' ); ?> static">
					<a href="<?php echo admin_url( 'admin.php?page=course_details&tab=units&course_id=' . $course_id . '&action=add_new_unit' ); ?>" class="<?php echo (!isset( $_GET[ 'unit_id' ] ) ? 'mp-tab-link' : 'button-secondary' ); ?>"><?php _e( 'Add new Unit', 'coursepress' ); ?></a>
				</li>
			<?php } ?>
        </ul>
		...
		<?php if ( CoursePress_Capabilities::can_create_course_unit( $course_id ) ) { ?>
			<!--<div class="mp-tabs">
				<div class="mp-tab <?php echo (!isset( $_GET[ 'unit_id' ] ) ? 'active' : '' ); ?>">
					<a href="?page=course_details&tab=units&course_id=<?php echo $course_id; ?>&action=add_new_unit" class="<?php echo (!isset( $_GET[ 'unit_id' ] ) ? 'mp-tab-link' : 'button-secondary' ); ?>"><?php _e( 'Add new Unit', 'coursepress' ); ?></a>
				</div>
			</div>-->
		<?php } ?>
        ...
        <form action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page . '&tab=units&course_id=' . $course_id . '&action=add_new_unit' . ( ( $unit_id !== 0 ) ? '&ms=uu' : '&ms=ua' ) . (isset( $preview_redirect_url ) && $preview_redirect_url !== '' ? '&preview_redirect_url=' . $preview_redirect_url : '' ) ) ); ?>" name="unit-add" id="unit-add" class="unit-add" method="post">
            ...
            <div class='section static'>
                <div class='unit-detail-settings'>
                    <h3><i class="fa fa-cog"></i> <?php _e( 'Unit Settings', 'coursepress' ); ?>
                        <div class="unit-state">
                            ...
                            <span class="draft <?php echo 'off' == $control_position ? 'on' : 'off'; ?>"><?php _e( 'Draft', 'coursepress' ); ?></span>
                            <div class="control <?php echo $can_publish ? '' : 'disabled'; ?> <?php echo $control_position; ?>">
                                <div class="toggle"></div>
                            </div>
                            <span class="live <?php echo 'on' == $control_position ? 'on' : 'off'; ?>"><?php _e( 'Live', 'coursepress' ); ?></span>
                        </div>
                    </h3>

                    <div class='mp-settings-label'><label for='unit_name'><?php _e( 'Unit Title', 'coursepress' ); ?></label></div>
                    ...
                    <div class='mp-settings-label'><label for='unit_availability'><?php _e( 'Unit Availability', 'coursepress' ); ?></label></div>
                    <div class='mp-settings-field'>
                        ...
                        <div class="force_unit_completion">
                            <input type="checkbox" name="force_current_unit_completion" id="force_current_unit_completion" value="on" <?php echo ( $force_current_unit_completion == 'on' ) ? 'checked' : ''; ?> /> <?php _e( 'User needs to <strong><em>answer</em></strong> all mandatory assessments and view all pages in order to access the next unit', 'coursepress' ); ?>
                        </div>						
                        <div class="force_unit_successful_completion">
							<input type="checkbox" name="force_current_unit_successful_completion" id="force_current_unit_successful_completion" value="on" <?php echo ( $force_current_unit_successful_completion == 'on' ) ? 'checked' : ''; ?> /> <?php _e( 'User also needs to <strong><em>pass</em></strong> all mandatory assessments', 'coursepress' ); ?>
						</div>
                        <div class="refresh_unit_completion_progress">
                            <input type="checkbox" name="refresh_unit_completion_progress" id="refresh_unit_completion_progress" value="on" /> <?php _e( 'Force unit completion refresh.', 'coursepress' ); ?>
                        </div>
                    </div>					
                </div>
                <div class="unit-control-buttons">
					...
					if ( $unit_id == 0 && CoursePress_Capabilities::can_create_course_unit( $course_id ) ) {//do not show anything
						...
						<input type="submit" name="submit-unit" class="button button-units save-unit-button" value="<?php _e( 'Save', 'coursepress' ); ?>">
					...
					if ( $unit_id != 0 && CoursePress_Capabilities::can_update_course_unit( $course_id, $unit_id ) ) {//do not show anything
						?>
						<input type="hidden" name="preview_redirect" value="<?php echo $preview_redirect; ?>" />
						<input type="submit" name="submit-unit" class="button button-units save-unit-button" value="<?php echo ( $unit_object->post_status == 'unpublished' ) ? __( 'Save', 'coursepress' ) : __( 'Save', 'coursepress' ); ?>">
					<?php } ?>
					...
					if ( CoursePress_Capabilities::can_update_course_unit( $course_id, $unit_id ) ) {//do not show anything if user can't update course unit
						?>
						<a class="button button-preview" href="<?php echo get_permalink( $unit_id ); ?>" data-href="<?php echo get_permalink( $unit_id ); ?>" target="_new"><?php _e( 'Preview', 'coursepress' ); ?></a>
						...
						/* if (current_user_can('coursepress_change_course_unit_status_cap') || ( current_user_can('coursepress_change_my_course_unit_status_cap') && $unit_object->post_author == get_current_user_id() )) { ?>
						  <input type="submit" name="submit-unit-<?php echo ( $unit_object->post_status == 'unpublished' ) ? 'publish' : 'unpublish'; ?>" class="button button-units button-<?php echo ( $unit_object->post_status == 'unpublished' ) ? 'publish' : 'unpublish'; ?>" value="<?php echo ( $unit_object->post_status == 'unpublished' ) ? __('Publish', 'coursepress') : __('Unpublish', 'coursepress'); ?>">
						  <?php
						  } */
					}
					?>
					...
					<?php if ( $unit_id != 0 ) { ?>
						<span class="delete_unit">							
							<a class="button button-units button-delete-unit" href="<?php echo admin_url( 'admin.php?page=course_details&tab=units&course_id=' . $course_id . '&unit_id=' . $unit_id . '&action=delete_unit' ); ?>" onclick="return removeUnit();">
								<i class="fa fa-trash-o"></i>&nbsp;&nbsp;&nbsp;<?php _e( 'Delete Unit', 'coursepress' ); ?>
							</a>
						</span>
					<?php } ?>
                    ...
                    <div id='edit-sub' class='course-holder-wrap elements-wrap'>
                        ...
                        <div class='course-holder'>
                            <!--<div class='course-details'>
                                ...
                                <label for='unit_description'><?php //_e('Introduction to this Unit', 'coursepress');                   ?></label>
							<?php
                            ...
                            <div class="module-droppable levels-sortable ui-droppable" style='display: none;'>
								<?php _e( 'Drag & Drop unit elements here', 'coursepress' ); ?>
                            </div>

                            <div id="unit-pages">
                                <ul class="sidebar-name unit-pages-navigation">
                                    <li class="unit-pages-title"><span><?php _e( 'Unit Page(s)', 'coursepress' ); ?></span></li>
									<?php
									if ( $unit_pagination ) {
									...
									<div id="unit-page-<?php echo $i; ?>" class='unit-page-holder'>
										<div class='course-details elements-holder'>
											<div class="unit_page_title">
												<label><?php _e( 'Page Label', 'coursepress' ); ?>
													<span class="delete_unit_page">							
														<a class="button button-units button-delete-unit"><i class="fa fa-trash-o"></i> <?php _e( 'Delete Page', 'coursepress' ); ?></a>
													</span>
												</label>
												<div class="description"><?php _e( 'The label will be displayed on the Course Overview and Unit page', 'coursepress' ); ?></div>
												...
												<label class="show_page_title">
													...
													<?php _e( 'Show page label on unit.', 'coursepress' ); ?><br />
												</label>

												<label><?php _e( 'Build Page', 'coursepress' ); ?></label>
												<div class="description"><?php _e( 'Click to add elements to the page', 'coursepress' ); ?></div>
											</div>
											<?php
											...
											<span class="no-elements"><?php _e( 'No elements have been added to this page yet', 'coursepress' ); ?></span>
										...
										<?php /* if ( is_array( $modules ) && count( $modules ) >= 1 ) {
										  ?>
										  <div class="loading_elements"><?php _e( 'Loading Unit elements, please wait...', 'coursepress' ); ?></div>
										  <?php } */ ?>
							...
							<div class="unit_pages_preloader">
								<div class="preloader_image"><?php _e( 'Loading unit elements...', 'coursepress' ); ?></div>
							</div>

							<div class="unit_pages_delete">
								<div class="unit_pages_delete_message"><?php _e( 'Deleting the unit page...', 'coursepress' ); ?></div>
							</div>
    ...
    <div class='level-liquid-right' style="display:none;">
        <div class="level-holder-wrap">
			<?php
			$sections = array( "input" => __( 'Input Elements', 'coursepress' ), "output" => __( 'Output Elements', 'coursepress' ), "invisible" => __( 'Invisible Elements', 'coursepress' ) );
