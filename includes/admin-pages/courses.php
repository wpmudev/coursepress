				...
				switch ( addslashes( $action ) ) {
					case 'publish':
						if ( CoursePress_Capabilities::can_change_course_status( $course_id ) ) {
							...
							$message		 = __( 'Selected courses have been published successfully.', 'coursepress' );
							$some_success	 = true;
						} else {
							if ( $some_success ) {
								$message = __( "Your selected courses have been published successfully. Courses where you don't have access remain unchaged.", 'coursepress' );
							} else {
								$message = __( "You don't have right permissions to change course status.", 'coursepress' );
							}
						}
						break;
					...
					case 'unpublish':
						if ( CoursePress_Capabilities::can_change_course_status( $course_id ) ) {
							...
							$message		 = __( 'Selected courses have been unpublished successfully.', 'coursepress' );
							$some_success	 = true;
						} else {
							if ( $some_success ) {
								$message = __( "Your selected courses have been unpublished successfully. Courses where you don't have access remain unchaged.", 'coursepress' );
							} else {
								$message = __( "You don't have right permissions to change course status.", 'coursepress' );
							}
						}
					...
					case 'delete':
						if ( CoursePress_Capabilities::can_delete_course( $course_id ) ) {
							...
							$message		 = __( 'Selected courses have been deleted successfully.', 'coursepress' );
							$some_success	 = true;
						} else {
							if ( $some_success ) {
								$message = __( "Your selected courses have been deleted successfully. Courses where you don't have access remain unchaged.", 'coursepress' );
							} else {
								$message = __( "You don't have right permissions to delete the course.", 'coursepress' );
							}
						}
						break;
				}
...
// Query the courses
	...
	if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'delete' && isset( $_GET[ 'course_id' ] ) && is_numeric( $_GET[ 'course_id' ] ) ) {
		...
		if ( CoursePress_Capabilities::can_delete_course( $_GET[ 'course_id' ] ) ) {
			...
			$message		 = __( 'Selected course has been deleted successfully.', 'coursepress' );
		} else {
			$message = __( "You don't have right permissions to delete the course.", 'coursepress' );
		}
	}
	...
	if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'change_status' && isset( $_GET[ 'course_id' ] ) && is_numeric( $_GET[ 'course_id' ] ) ) {
		...
		$message = __( 'Status for the selected course has been changed successfully.', 'coursepress' );
	}
	?>
	<div class="wrap nosubsub cp-wrap">
			...
			if ( CoursePress_Capabilities::can_create_course() ) {
				if ( $wp_course_search->is_light ) {
					if ( $wp_course_search->get_count_of_all_courses() < $wp_course_search->courses_per_page ) {
						?>
						<a class="add-new-h2" href="<?php echo admin_url( 'admin.php?page=course_details' ); ?>"><?php _e( 'Add New', 'coursepress' ); ?></a>
						<?php
					}
				} else {
					?>
			...
			<div class="alignright actions new-actions">
				<form method="get" action="<?php echo admin_url( 'admin.php?page=' . $page ); ?>" class="search-form">
					<p class="search-box">
						...
						<label class="screen-reader-text"><?php _e( 'Search Courses', 'coursepress' ); ?>:</label>
						<input type="text" value="<?php echo esc_attr( $s ); ?>" name="s">
						<input type="submit" class="button" value="<?php _e( 'Search Courses', 'coursepress' ); ?>">
					</p>
				</form>
			</div>
			<!--/alignright-->
					...
					<div class="alignleft actions">
						<select name="action">
							...
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_change_course_status_cap' ) ) { ?>
								<option value="publish"><?php _e( 'Publish', 'coursepress' ); ?></option>
								<option value="unpublish"><?php _e( 'Unpublish', 'coursepress' ); ?></option>
							<?php } ?>
						...
						<input type="submit" class="button-secondary action" id="doaction_bulk_courses" name="doaction" value="<?php _e( 'Apply', 'coursepress' ); ?>"/>
					</div>
				<?php } ?>
			</form>
			<form id="posts-filter" action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page ) ); ?>" method="get">
				<div class="alignleft actions">
					...
					<select name="course_category_filter" id="cat" class="postform">
						...
						?>
						<option value="0" <?php selected( $category_filter_val, 0, true ); ?>><?php _e( 'View all categories', 'coursepress' ); ?></option>
						<?php
					...
					<input type="submit" name="filter_action" id="post-query-submit" class="button" value="<?php _e( 'Filter', 'coursepress' ); ?>">
				</div>
			</form>
		...
		$columns = array(
			...
			"status"	 => __( 'Published', 'coursepress' ),
		...
		$columns[ "remove" ] = __( 'Delete', 'coursepress' );
		$col_sizes[]		 = '7';
		?>
		...
		<table cellspacing="0" class="widefat shadow-table unit-control-buttons" id="courses_table">
			...
			?>">
					...
					<!-- <div class="course-thumbnail"><img src="<?php echo Course::get_course_thumbnail( $course->ID ); ?>" alt="<?php echo esc_attr( $course_object->post_title ); ?>" /></div> -->
							...
							<div class="column-course-units visible-small visible-extra-small">
								<strong><?php _e( 'Units', 'coursepress' ); ?>:</strong>
								<?php echo $course_obj->get_units( '', 'any', true ); ?> <?php _e( 'Units', 'coursepress' ); ?>,
								<?php echo $course_obj->get_units( '', 'publish', true ); ?> Published
							</div>
							<div class="column-course-students visible-small visible-extra-small">
								<strong><?php _e( 'Students', 'coursepress' ); ?>:</strong>
							...
							<div class="row-actions hide-small hide-extra-small">
								<?php if ( $can_update ) { ?>
									<span class="edit_course"><a href="<?php echo admin_url( 'admin.php?page=course_details&course_id=' . $course_object->ID ); ?>"><?php _e( 'Edit', 'coursepress' ); ?></a> | </span>
								<?php } ?>
								<?php if ( $can_view_unit || $my_course ) { ?>
									<span class="course_units"><a href="<?php echo admin_url( 'admin.php?page=course_details&tab=units&course_id=' . $course_object->ID ); ?>"><?php _e( 'Units', 'coursepress' ); ?></a> | </span>
								<?php } ?>
								<?php if ( $can_update || $my_course ) { ?>
									<span class="course_students"><a href="<?php echo admin_url( 'admin.php?page=course_details&tab=students&course_id=' . $course_object->ID ); ?>"><?php _e( 'Students', 'coursepress' ); ?></a> | </span>
								<?php } ?>
								<span class="view_course"><a href="<?php echo get_permalink( $course->ID ); ?>" rel="permalink"><?php _e( 'View Course', 'coursepress' ) ?></a>
									<?php if ( $can_view_unit || $my_course || $can_update ) { ?> | <?php } ?></span>
								<?php if ( $can_view_unit || $my_course || $can_update ) { ?>
									<span class="units"><a href="<?php echo trailingslashit( get_permalink( $course->ID ) ) . trailingslashit( $coursepress->get_units_slug() ); ?>" rel="permalink"><?php _e( 'View Units', 'coursepress' ) ?></a></span>
								<?php } ?>
								<?php
								if ( CoursePress_Capabilities::can_create_course() ) {
									if ( $wp_course_search->is_light ) {
										if ( $wp_course_search->get_count_of_all_courses() < $wp_course_search->courses_per_page ) {
											?>
											|
											<span class="units"><a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=courses&course_action=duplicate&course_id=' . $course_object->ID ), 'duplicating_course', 'duplicating_nonce' ); ?>"><?php _e( 'Duplicate Course', 'coursepress' ) ?></a></span>
											<?php
										}
									} else {
										?>
						...
						<td class="column-units <?php echo $style; ?>">
							<?php echo $course_obj->get_units( '', 'any', true ); ?> <?php _e( 'Units', 'coursepress' ); ?><br/>
							<?php echo $course_obj->get_units( '', 'publish', true ); ?> <?php _e( 'Published', 'coursepress' ); ?>
						</td>
			...
			if ( count( $wp_course_search->get_results() ) == 0 ) {
				...
				<tr>
					<td colspan="6">
						<div class="zero-courses"><?php _e( 'No courses found.', 'coursepress' ) ?></div>
					</td>
				</tr>
				<?php
			}
			...
			<form id="posts-filter" action="" method="get">
				<div class="alignleft actions bulkactions">
					...
					<select name="courses_per_page" id="courses_per_page">
						<option value="10" <?php selected( $show_courses_per_page, 10, true ); ?>><?php _e( 'Show 10 rows', 'coursepress' ); ?></option>
						<option value="20" <?php selected( $show_courses_per_page, 20, true ); ?>><?php _e( 'Show 20 rows', 'coursepress' ); ?></option>
						<option value="30" <?php selected( $show_courses_per_page, 30, true ); ?>><?php _e( 'Show 30 rows', 'coursepress' ); ?></option>
						<option value="40" <?php selected( $show_courses_per_page, 40, true ); ?>><?php _e( 'Show 40 rows', 'coursepress' ); ?></option>
						<option value="50" <?php selected( $show_courses_per_page, 50, true ); ?>><?php _e( 'Show 50 rows', 'coursepress' ); ?></option>
						<option value="60" <?php selected( $show_courses_per_page, 60, true ); ?>><?php _e( 'Show 60 rows', 'coursepress' ); ?></option>
						<option value="70" <?php selected( $show_courses_per_page, 70, true ); ?>><?php _e( 'Show 70 rows', 'coursepress' ); ?></option>
						<option value="80" <?php selected( $show_courses_per_page, 80, true ); ?>><?php _e( 'Show 80 rows', 'coursepress' ); ?></option>
						<option value="90" <?php selected( $show_courses_per_page, 90, true ); ?>><?php _e( 'Show 90 rows', 'coursepress' ); ?></option>
						<option value="100" <?php selected( $show_courses_per_page, 100, true ); ?>><?php _e( 'Show 100 rows', 'coursepress' ); ?></option>
					</select>
					<input type="submit" name="" class="button action" value="<?php esc_attr_e( 'Apply', 'coursepress' ); ?>">
				</div>
			</form>
