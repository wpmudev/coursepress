...
if ( current_user_can( 'manage_options' ) || ( current_user_can( 'coursepress_withdraw_students_cap' ) ) || ( current_user_can( 'coursepress_withdraw_my_students_cap' ) && $course->details->post_author == get_current_user_id() ) ) {
	$columns["delete"] = __( 'Withdraw', 'coursepress' );
	$col_sizes[]       = '12';
}
...
<div id="students_accordion" class="cp-wrap">
	...
	<div class="sidebar-name no-movecursor">
		<h3 data-title="<?php _e( 'Default', 'coursepress' ); ?>"><?php _e( 'Default', 'coursepress' ); ?>
			...
			<table cellspacing="0" class="widefat">
				...
				foreach ( $wp_user_search->get_results() as $user ) {
					...
					<tr id='user-<?php echo $user_object->ID; ?>' <?php echo $style; ?>>
						...
						<!--<td class="<?php echo $style; ?>"><?php echo( $user_object->{'enrolled_course_group_' . $course_id} == '' ? __( 'Default', 'coursepress' ) : $user_object->{'enrolled_course_group_' . $course_id} ); ?></td>-->
				...
				<?php if ( current_user_can( 'manage_options' ) || ( current_user_can( 'coursepress_add_move_students_cap' ) ) || ( current_user_can( 'coursepress_add_move_my_students_cap' ) && $course->details->post_author == get_current_user_id() ) || ( current_user_can( 'coursepress_add_move_my_assigned_students_cap' ) && $my_course ) ) { ?>
					...
					?>" action="<?php echo admin_url( 'admin.php?page=course_details&tab=students&course_id=' . $course_id . '&ms=as' ); ?>" method="post">
						...
						<?php if ( $students->total_users > 0 ) { ?>
							<?php cp_students_drop_down(); ?> <?php submit_button( __( 'Add Student', 'coursepress' ), 'secondary', 'add_new_student', '' ); ?>
						<?php } ?>
	...
	<?php } else { ?>
		<div>
			...
			<table cellspacing="0" class="widefat">
				<tr>
					<td>
						<div class="zero-students"><?php _e( '0 Students in this course', 'coursepress' ); ?></div>
					</td>
				</tr>
			</table>
	...
	/* if ( !empty( $course_classes ) ) {
	  ...
	  <a href="?page=course_details&tab=students&course_id=<?php echo $course_id; ?>&delete_class=<?php echo urlencode( ( isset( $class ) ? $class : '' ) ); ?>" onClick="return deleteClass();" title="<?php _e( 'Delete Class and move students to Default class', 'coursepress' ); ?>"><?php _e( 'Delete Class', 'coursepress' ); ?></a>
	  <?php } ?>
	  ...
	  <?php } ?>
	  <?php if ( ( current_user_can( 'coursepress_withdraw_students_cap' ) ) || ( current_user_can( 'coursepress_withdraw_my_students_cap' ) && $course->details->post_author == get_current_user_id() ) ) { ?>
	  <a href="?page=course_details&tab=students&course_id=<?php echo $course_id; ?>&withdraw_all=<?php echo urlencode( ( isset( $class ) ? $class : '' ) ); ?>" onClick="return withdrawAllFromClass();" title="<?php _e( 'Withdraw all students from the course', 'coursepress' ); ?>"><?php _e( 'Withdraw all students', 'coursepress' ); ?></a>
	  <?php } ?>
	  </div>
	  ...
	  <tr>
	  <td>
	  <div class="zero-students"><?php _e( '0 Students in this class', 'coursepress' ); ?></div>
	  </td>
	  </tr>
	  </table>
	  ...
	  <div class="additional_class_actions">
	  <?php if ( ( current_user_can( 'coursepress_delete_classes_cap' ) ) || ( current_user_can( 'coursepress_delete_my_classes_cap' ) && $course->details->post_author == get_current_user_id() ) ) { ?>
	  <a href="?page=course_details&tab=students&course_id=<?php echo $course_id; ?>&delete_class=<?php echo urlencode( $class ); ?>" onClick="return deleteClass();" title="<?php _e( 'Delete Class', 'coursepress' ); ?>"><?php _e( 'Delete Class', 'coursepress' ); ?></a>
	  <?php } ?>
	  </div>
...
<form name="" method="post" action="<?php echo admin_url( 'admin.php?page=course_details&tab=students&course_id=' . $course_id . '&ms=ac' ); ?>">
	...
	<?php if ( ( ( current_user_can( 'coursepress_add_new_classes_cap' ) ) || ( current_user_can( 'coursepress_add_new_my_classes_cap' ) && $course->details->post_author == get_current_user_id() ) ) && 1 == 0 /* moving class feature for the next release */ ) { ?>
		<div class="add-student-class-area">
			<h2><?php _e( 'New Class', 'coursepress' ); ?></h2>
			<label><?php _e( 'New Class name', 'coursepress' ); ?>
				<input type="text" name="course_classes[]" class="course_classes_input" value=""/>
			</label>

			<?php submit_button( __( 'Add New Class', 'coursepress' ), 'primary', 'add_student_class', '' ); ?>
	...
	<?php if ( current_user_can( 'manage_options' ) || ( current_user_can( 'coursepress_invite_students_cap' ) ) || ( current_user_can( 'coursepress_invite_my_students_cap' ) && $course->details->post_author == get_current_user_id() ) ) { ?>
		<div class="invite_student_area">
			<form name="student_invitation" method="post" class='student-invitation'>
				...
				<h2><?php _e( 'Invite a Student', 'coursepress' ); ?></h2>
				<label><span><?php _e( 'First Name', 'coursepress' ); ?></span>
					<input type="text" name="first_name" value=""/>
				</label>

				<label><span><?php _e( 'Last Name', 'coursepress' ); ?></span>
					<input type="text" name="last_name" value=""/>
				</label>

				<label><span><?php _e( 'E-Mail', 'coursepress' ); ?></span>
					<input type="text" name="email" value=""/>
				</label>
				<?php submit_button( __( 'Invite', 'coursepress' ), 'primary', 'invite_student', '' ); ?>
			</form>
		</div>
	<?php } ?>
<?php } ?>
...
<?php /*
  ...
  <h2><?php _e( 'Send an e-mail notification to students', 'coursepress' ); ?></h2>

  <label class="email_subject"><?php _e( 'E-Mail Subject', 'coursepress' ); ?>
  <input type="text" name="email_subject" value="" />
  </label>
  <label class="email_body"><?php _e( 'E-Mail Body', 'coursepress' ); ?>
  <?php
  $args = array( "textarea_name" => "email_body", "textarea_rows" => 3 );
  wp_editor( '', "email_body", $args );
  ?>
  </label><br />
  <?php submit_button( __( 'Send', 'coursepress' ), 'primary', 'send_bulk_email_to_students', '' ); ?>
  </form>
  </div>
  <?php } */ ?>
