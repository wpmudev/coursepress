			...
			switch ( addslashes( $action ) ) {
				case 'delete':
					if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_delete_students_cap' ) ) {
						...
						$message = __( 'Selected students have been withdrawn from all courses successfully. Note: The user accounts still exist.', '<%= wpmudev.plugin.textdomain %>' );
					}
					break;
				...
				case 'withdraw':
					if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_withdraw_students_cap' ) ) {
						...
						$message = __( 'Selected students have been withdrawn from all courses successfully.', '<%= wpmudev.plugin.textdomain %>' );
					}
					break;
			}
		}
...
if ( isset( $_GET['action'] ) && $_GET['action'] == 'delete' && isset( $_GET['student_id'] ) && is_numeric( $_GET['student_id'] ) ) {
	...
	$student->delete_student();
	$message = __( 'Selected student has been withdrawn from all courses successfully. Note: The user account still exists.', '<%= wpmudev.plugin.textdomain %>' );
}
...
// Query the users
	...
	<div class="wrap nosubsub students cp-wrap">
		...
		<h2><?php _e( 'Students', '<%= wpmudev.plugin.textdomain %>' ); ?><?php if ( current_user_can( 'manage_options' ) ) { ?>
				<a class="add-new-h2" href="user-new.php"><?php _e( 'Add New', '<%= wpmudev.plugin.textdomain %>' ); ?></a><?php } ?><?php if ( current_user_can( 'coursepress_add_new_students_cap' ) && ! current_user_can( 'manage_options' ) ) { ?>
				<a class="add-new-h2" href="<?php echo admin_url( 'admin.php?page=students&action=add_new' ); ?>"><?php _e( 'Add New', '<%= wpmudev.plugin.textdomain %>' ); ?></a><?php } ?>
		</h2>
			...
			<div class="alignright actions new-actions">
				<form method="get" action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page ) ); ?>" class="search-form">
					<p class="search-box">
						...
						<label class="screen-reader-text"><?php _e( 'Search Students', '<%= wpmudev.plugin.textdomain %>' ); ?>:</label>
						<input type="text" value="<?php echo esc_attr( $s ); ?>" name="s">
						<input type="submit" class="button" value="<?php _e( 'Search Students', '<%= wpmudev.plugin.textdomain %>' ); ?>">
					</p>
				</form>
			</div>
					...
					<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_withdraw_students_cap' ) || current_user_can( 'coursepress_delete_students_cap' ) ) { ?>
						<select name="action">
							...
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_withdraw_students_cap' ) ) { ?>
								<option value="withdraw"><?php _e( 'Withdraw from all courses', '<%= wpmudev.plugin.textdomain %>' ); ?></option>
							<?php } ?>
						</select>
		...
		$columns = array(
			...
			"registration_date" => __( 'Registered', '<%= wpmudev.plugin.textdomain %>' ),
			"latest_activity"   => __( 'Latest Activity', '<%= wpmudev.plugin.textdomain %>' ),
			"courses"           => __( 'Courses', '<%= wpmudev.plugin.textdomain %>' ),
			"workbook"          => __( 'Workbook', '<%= wpmudev.plugin.textdomain %>' ),
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_delete_students_cap' ) ) {
			$columns["delete"] = __( 'Remove', '<%= wpmudev.plugin.textdomain %>' );
			$col_sizes[]       = '5';
		}
		?>
				...
				<tr id='user-<?php echo $user_object->ID; ?>' class="<?php echo $style; ?>">
					...
					<td class="column-user-fullname visible-small visible-extra-small <?php echo $style; ?>">
						...
						<div class="visible-extra-small">
							<?php _e( 'Latest Activity:', '<%= wpmudev.plugin.textdomain %>' ); ?>
							<span class="latest_activity"><?php echo( isset( $user_object->latest_activity ) && $user_object->latest_activity !== '' ? date_i18n( 'Y-m-d h:i:s', $user_object->latest_activity ) : __( 'N/A', '<%= wpmudev.plugin.textdomain %>' ) ); ?></span> <?php if ( $coursepress->user_is_currently_active( $user_object->ID ) ) { ?>
								<a class="activity_circle" alt="<?php _e( 'User is currently active on the website', '<%= wpmudev.plugin.textdomain %>' ); ?>"  title="<?php _e( 'User is currently active on the website', '<%= wpmudev.plugin.textdomain %>' ); ?>"></a><?php } ?>
						</div>
					</td>
