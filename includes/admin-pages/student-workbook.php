...
<div class="wrap nocoursesub student-workbook cp-wrap">
	...
	<h2><?php _e( 'Student Workbook', 'coursepress' ); ?></h2>
				...
				<div class="course-holder">
					...
					<div class='student-profile-info'>
						...
						<div class="student_additional_info">
							...
							<div>
								<span class="info_caption"><?php _e( 'Profile', 'coursepress' ); ?></span>
										...
										<div class="alignleft actions">
											<select name="course_id" id="dynamic_courses" class="chosen-select">
												...
												if ( $courses_with_students == 0 ) {
													?>
													<option value=""><?php _e( 'Student did not enroll into any course yet.', 'coursepress' ); ?></option>
												<?php
												}
												?>
											</select>
																...
																?>">
																	...
																	if ( $general_col_visibility ) {
																		...
																		<td class="coloumn-submission-date <?php echo $style . ' ' . $visibility_class; ?>">
																			<?php echo( count( $response ) >= 1 ? $response->post_date : __( 'Not submitted', 'coursepress' ) ); ?>
																		</td>
																		...
																		<td class="column-response <?php echo $style . ' ' . $visibility_class; ?>">
																			...
																			if ( count( $response ) >= 1 ) {
																				...
																				<a class="assessment-view-response-link button button-units" href="<?php echo admin_url( 'admin.php?page=assessment&course_id=' . $current_course_id . '&unit_id=' . $unit_id . '&user_id=' . $user_object->ID . '&module_id=' . $mod->ID . '&response_id=' . $response->ID . '&assessment_page=' . $assessment_page ); ?>"><?php _e( 'View', 'coursepress' ); ?></a>
																			...
																			if ( $assessable == 'yes' ) {
																				...
																				if ( count( $response ) >= 1 ) {
																					if ( isset( $grade_data ) ) {
																						...
																						echo sprintf( __( ' on %s', 'coursepress' ), $grade_time );
																						?>" title="<?php
																						_e( 'Grade by ', 'coursepress' );
																						echo $instructor_name->display_name;
																						echo sprintf( __( ' on %s', 'coursepress' ), $grade_time );
