			...
			if ( ! email_exists( $_POST['email'] ) ) {
				...
				if ( $form_errors == 0 ) {
					if ( $student_id = $student->add_student( $student_data ) !== 0 ) {
						...
						$form_message       = __( 'Account created successfully!', '<%= wpmudev.plugin.textdomain %>' );
						...
						$form_message       = __( 'An error occured while creating the account. Please check the form and try again.', '<%= wpmudev.plugin.textdomain %>' );
						$form_message_class = 'error';
					}
				}
			} else {
...
<form id="student-settings" name="student-settings" method="post" class="student-settings cp-wrap">
	<div class="wrap nosubsub">
		...
		<h2><?php _e( 'Add New Student', '<%= wpmudev.plugin.textdomain %>' ); ?></h2>
		...
		<div id="edit-sub" class="course-holder-wrap">
			...
			<div class="sidebar-name no-movecursor">
				<h3><?php _e( 'Course Details', '<%= wpmudev.plugin.textdomain %>' ); ?></h3>
			</div>
			...
			<div class="course-holder">
				<div class="course-details">
					<div class="half">
						<label>
							<?php _e( 'First Name', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
					...
					<div class="half">
						<label>
							<?php _e( 'Last Name', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
					...
					<div class="half">
						<label>
							<?php _e( 'Username', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
					...
					<div class="half">
						<label>
							<?php _e( 'E-mail', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
					...
					<div class="half">
						<label>
							<?php _e( 'Password', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
					...
					<div class="half">
						<label class="left">
							<?php _e( 'Confirm Password', '<%= wpmudev.plugin.textdomain %>' ); ?>:<br/>
	...
	<p class="save-shanges">
		<?php submit_button( __( 'Add New Student', '<%= wpmudev.plugin.textdomain %>' ) ); ?>
	</p>
</form>