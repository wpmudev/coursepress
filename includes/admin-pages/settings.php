...
if ( empty( $tab ) ) {
	if ( current_user_can( 'manage_options' ) ) {
		...
		?>
		<div id="error-page">
			<p><?php _e( 'You do not have required permissions to access Settings.', 'coursepress' ); ?></p>
		</div>
		<?php
		exit;
		//die( __( 'You do not have required permissions to access Settings.', 'coursepress' ) );
	}
}
...
<div class='wrap mp-wrap nocoursesub cp-wrap' id="settings-wrap">
	...
	<h2><?php _e( 'Settings', 'coursepress' ); ?></h2>

	<?php
	if ( isset( $_POST['submit'] ) ) {
		?>
		<div id="message" class="updated fade"><p><?php _e( 'Settings saved successfully.', 'coursepress' ); ?></p></div>
	<?php
	}
	?>
	...
	$menus = array();
	if ( current_user_can( 'manage_options' ) ) {
		$menus['general'] = __( 'General', 'coursepress' );
	}

	/* if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_settings_groups_page_cap' ) ) {
	  //$menus['groups'] = __( 'Class Groups', 'coursepress' ); //to do in the next release
	  } */

	if ( current_user_can( 'manage_options' ) ) {
		/* $menus['payment'] = __( 'Payment Settings', 'coursepress' ); */
		$menus['email'] = __( 'E-mail Settings', 'coursepress' );
	}

	if ( current_user_can( 'manage_options' ) ) {
		$menus['instructor_capabilities'] = __( 'Instructor Capabilities', 'coursepress' );
	}

	if ( current_user_can( 'coursepress_settings_shortcode_page_cap' ) || current_user_can( 'manage_options' ) ) {
		$menus['shortcodes'] = __( 'Shortcodes', 'coursepress' );
	}

	if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
		$menus['cp-marketpress'] = __( 'MarketPress', 'coursepress' );
	}
	...
	<div id="undefined-sticky-wrapper" class="sticky-wrapper">
		...
		<ul class="mp-tabs" style="">
			...
			?>
			<li class="mp-tab">
				<a class="mp-tab-link" href="<?php echo admin_url( 'admin.php?page=courses&quick_setup' ); ?>"><?php _e( 'View Setup Guide', 'coursepress' ); ?></a>
			</li>
		</ul>
	</div>
