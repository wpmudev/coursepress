				...
				switch ( addslashes( $action ) ) {
					case 'publish':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_change_notification_status_cap' ) || ( current_user_can( 'coursepress_change_my_course_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
							...
							$message = __( 'Selected notifications have been published successfully.', 'coursepress' );
						} else {
							$message = __( "You don't have right permissions to change notification status.", 'coursepress' );
						}
						break;
					...
					case 'unpublish':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_change_notification_status_cap' ) || ( current_user_can( 'coursepress_change_my_notification_status_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
							...
							$message = __( 'Selected notifications have been set to private successfully.', 'coursepress' );
						} else {
							$message = __( "You don't have right permissions to change notification status.", 'coursepress' );
						}
						break;
					...
					case 'delete':
						if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_delete_notification_cap' ) || ( current_user_can( 'coursepress_delete_my_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
							...
							$message = __( 'Selected notifications have been deleted successfully.', 'coursepress' );
						} else {
							$message = __( "You don't have right permissions to delete the notification.", 'coursepress' );
						}
						break;
				}
			}
...
// Query the notifications
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] == 'delete' && isset( $_GET['notification_id'] ) && is_numeric( $_GET['notification_id'] ) ) {
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_delete_notification_cap' ) || ( current_user_can( 'coursepress_delete_my_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) {
			...
			$message = __( 'Selected notification has been deleted successfully.', 'coursepress' );
		} else {
			$message = __( "You don't have right permissions to delete the notification.", 'coursepress' );
		}
	}
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] == 'change_status' && isset( $_GET['notification_id'] ) && is_numeric( $_GET['notification_id'] ) ) {
		...
		$message = __( 'Status for the selected notification has been changed successfully.', 'coursepress' );
	}
	?>
	<div class="wrap nosubsub notifications cp-wrap">
		<div class="icon32" id="icon-themes"><br></div>
		<h2><?php _e( 'Notifications', 'coursepress' ); ?><?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_create_notification_cap' ) || current_user_can( 'coursepress_create_my_notification_cap' ) || current_user_can( 'coursepress_create_my_assigned_notification_cap' ) ) { ?>
				<a class="add-new-h2" href="<?php echo admin_url( 'admin.php?page=notifications&action=add_new' ); ?>"><?php _e( 'Add New', 'coursepress' ); ?></a><?php } ?>
		</h2>
		<?php
		$ms['add'] = __( 'New notification added successfully!', 'coursepress' );
		$ms['nu']  = __( 'Notification updated successfully.', 'coursepress' );
			...
			<div class="alignright actions new-actions">
				<form method="get" action="<?php echo esc_attr( admin_url( 'admin.php?page=' . $page ) ); ?>" class="search-form">
					<p class="search-box">
						...
						<label class="screen-reader-text"><?php _e( 'Search Notifications', 'coursepress' ); ?>:</label>
						<input type="text" value="<?php echo esc_attr( $s ); ?>" name="s">
						<input type="submit" class="button" value="<?php _e( 'Search Notifications', 'coursepress' ); ?>">
					</p>
				</form>
			</div>
			<!--/alignright-->
					...
					<div class="alignleft actions">
						<select name="action">
							...
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_change_notification_status_cap' ) ) { ?>
								...
								<option value="unpublish"><?php _e( 'Private', 'coursepress' ); ?></option>
							<?php } ?>
		...
		$columns = array(
			"notification_title" => __( 'Notification', 'coursepress' ),
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_delete_notification_cap' ) || ( current_user_can( 'coursepress_delete_my_notification_cap' ) ) ) {
			$columns["remove"] = __( 'Remove', 'coursepress' );
			$col_sizes[]       = '7';
		}
		?>
		...
		<table cellspacing="0" class="widefat shadow-table">
			...
			foreach ( $wp_notification_search->get_results() as $notification ) {
				...
				if ( isset( $notification_object->course_id ) && $notification_object->course_id !== '' ) {
					...
					$course_name = __( 'All Courses', 'coursepress' );
				}
				?>
				<tr id='user-<?php echo $notification_object->ID; ?>' class="<?php echo $style; ?>">
					...
					<td class="column-notification-title <?php echo $style; ?>">
						...
						<div class="visible-small visible-extra-small">
							<strong><?php _e( 'Course:', 'coursepress' ); ?></strong> <?php echo $course_name; ?></div>
						<div class="visible-small visible-extra-small">
							<strong><?php _e( 'Status:', 'coursepress' ); ?></strong> <?php echo ( $notification_object->post_status == 'publish' ) ? ucfirst( $notification_object->post_status ) . 'ed' : ucfirst( $notification_object->post_status ); ?>
						</div>
						<div class="course_excerpt"><?php echo cp_get_the_course_excerpt( $notification_object->ID ); ?></div>
						<div class="row-actions">
							<span class="edit_notification"><a href="<?php echo admin_url( 'admin.php?page=notifications&action=edit&notification_id=' . $notification_object->ID ); ?>"><?php _e( 'Edit', 'coursepress' ); ?></a> | </span>
							...
							<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'coursepress_delete_notification_cap' ) || ( current_user_can( 'coursepress_delete_my_notification_cap' ) && $notification_object->post_author == get_current_user_id() ) ) { ?>
								<span class="course_remove"><a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=notifications&action=delete&notification_id=' . $notification_object->ID ), 'delete_notification_' . $notification_object->ID, 'cp_nonce' ); ?>" onClick="return removeNotification();"><?php _e( 'Delete', 'coursepress' ); ?></a> | </span>
							<?php } ?>
						</div>
					</td>
			...
			if ( count( $wp_notification_search->get_results() ) == 0 ) {
				...
				<tr>
					<td colspan="6">
						<div class="zero-courses"><?php _e( 'No notifications found.', 'coursepress' ) ?></div>
					</td>
				</tr>
			<?php
			}
