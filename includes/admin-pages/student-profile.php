...
<div class="wrap nocoursesub cp-wrap">
	...
	<h2><?php _e( 'Student Profile', '<%= wpmudev.plugin.textdomain %>' ); ?></h2>
					...
					<div class="course-holder">
						...
						<div class='student-profile-info'>
							...
							<div class="student_additional_info">
								...
								<div>
									<span class="info_caption"><?php _e( 'Username', '<%= wpmudev.plugin.textdomain %>' ); ?></span>
						...
						<?php
						$columns = array(
							"course"          => __( ' ', '<%= wpmudev.plugin.textdomain %>' ),
							"additional_info" => __( ' ', '<%= wpmudev.plugin.textdomain %>' ),
						);
						?>

						<div class="courses" id="student-profile-courses">
							<div class="sidebar-name no-movecursor">
								<h3><?php _e( 'Courses', '<%= wpmudev.plugin.textdomain %>' ); ?></h3>
									...
									if ( $course_object ) {
										...
										<div class="student-course">
											...
											<div class="student-course-top">
												<a href="<?php echo admin_url( 'admin.php?page=students&action=workbook&student_id=' . $student->ID . '&course_id=' . $course_object->ID ); ?>" class="button button-units workbook-button"><?php _e( 'View Workbook', '<%= wpmudev.plugin.textdomain %>' ); ?>
											...
											<div class="student-course-bottom">
												...
												<div class="course-info-holder">
													<span class="course_info_caption"><?php _e( 'Start', '<%= wpmudev.plugin.textdomain %>' ); ?>
														<i class="fa fa-calendar"></i></span>
                                                    <span class="course_info">
                                                        <?php
                                                        if ( $course_object->open_ended_course == 'on' ) {
	                                                        _e( 'Open-ended', '<%= wpmudev.plugin.textdomain %>' );
                                                        } else {
													...
													<span class="course_info_caption"><?php _e( 'End', '<%= wpmudev.plugin.textdomain %>' ); ?>
														<i class="fa fa-calendar"></i></span>
                                                    <span class="course_info">
                                                        <?php
                                                        if ( $course_object->open_ended_course == 'on' ) {
													...
													<span class="course_info_caption"><?php _e( 'Duration', '<%= wpmudev.plugin.textdomain %>' ); ?>
                                                    ...
                                                    <span class="course_info">
                                                        ...
                                                        ?> <?php _e( 'Days', '<%= wpmudev.plugin.textdomain %>' ); ?>
                                                    </span>
												</div>
													...
													<form name="form_student_<?php echo $course_object->ID; ?>" id="form_student_<?php echo $course_object->ID; ?>" method="post" action="<?php echo admin_url( 'admin.php?page=students&action=view&student_id=' . $student->ID ); ?>">
														...
														<div class="changable">
															<label class="class-label">
																<?php _e( 'Class', '<%= wpmudev.plugin.textdomain %>' ); ?>

																<select name="course_class" data-placeholder="'<?php _e( 'Choose a Class...', '<%= wpmudev.plugin.textdomain %>' ); ?>'" id="course_class_<?php echo $course_object->ID; ?>">

																	<option value=""<?php echo( $student->{'enrolled_course_class_' . $course_object->ID} == '' ? ' selected="selected"' : '' ); ?>><?php _e( 'Default', '<%= wpmudev.plugin.textdomain %>' ); ?></option>
																	<?php
															...
															<label class="group-label">
																<?php _e( 'Group', '<%= wpmudev.plugin.textdomain %>' ); ?>
																<select name="course_group" id="course_group_<?php echo $course_object->ID; ?>" data-placeholder="<?php esc_attr_e( 'Choose a Group...', '<%= wpmudev.plugin.textdomain %>' ); ?>">
																	<option value=""<?php echo( $student->{'enrolled_course_group_' . $course_object->ID} == '' ? ' selected="selected"' : '' ); ?>><?php _e( 'Default', '<%= wpmudev.plugin.textdomain %>' ); ?></option>
																	<?php
															...
															<?php submit_button( __( 'Save Changes', '<%= wpmudev.plugin.textdomain %>' ), 'secondary', 'save-group-class-changes', '' ) ?>
