			...
			if ( ! empty( $course_list ) ) {
				$content .= '
					<div class="dashboard-managed-courses-list">
						<h1 class="title managed-courses-title">' . esc_html__( 'Courses you manage:', CoursePress::TD ) . '</h1>
			...
			if ( empty( $course_list ) && $show_random_courses ) {
				...
				$content .= '
					<div class="dashboard-random-courses-list">
						...
						esc_html__( 'Here are a few to help you get started:', CoursePress::TD ) . '
						<hr />
				...
				$content .= '
					<div class="dashboard-current-courses-list">
						<h1 class="title enrolled-courses-title current-courses-title">' . __( 'Your current courses:', CoursePress::TD ) . '</h1>
			...
			if ( ! empty( $course_list ) ) {
				...
				$content .= '
					<div class="dashboard-completed-courses-list">
						<h1 class="title completed-courses-title">' . __( 'Completed courses:', CoursePress::TD ) . '</h1>
	...
	public static function student_settings() {
		...
		if ( is_user_logged_in() ) {
			...
			<form id="student-settings" name="student-settings" method="post" class="student-settings">' .
				...
				<label>
					' . esc_html__( 'Password', CoursePress::TD ) . ':
					<input type="password" name="password" value="" placeholder="' . esc_html__( "Won't change if empty.", CoursePress::TD ) .'"/>
				</label>
				' . do_action( 'coursepress_after_settings_passwordon' ) . '
				<label>
					' . esc_html__( 'Confirm Password', CoursePress::TD ) . ':
					<input type="password" name="password_confirmation" value=""/>
				</label>
				' . do_action( 'coursepress_after_settings_pasword' ) . '
				<label class="full">
					<input type="submit" name="student-settings-submit" class="apply-button-enrolled" value="' . esc_html__( 'Save Changes', CoursePress::TD ) .'"/>
				</label>
			</form>
			';
