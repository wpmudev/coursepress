...
abstract class CoursePress_View_Admin_Setting_Setting {
	...
	public static function process_form( $page, $tab ) {
		...
		if ( isset( $_POST['action'] ) && 'updateoptions' === $_POST['action'] && self::$slug === $tab && wp_verify_nonce( $_POST['_wpnonce'], 'update-coursepress-options' ) ) {
	...
	protected static function page_start( $slug, $tab ) {
		...
		$content .= '<table class="form-table slug-settings"><tbody>';
		return $content;
	 ...
	 * @return string Content part.
	 */
	protected static function page_end() {
