	...
	public static function render() {
		...
		if ( CoursePress_Data_Capabilities::can_invite_students( $course_id ) ) {
			...
			if ( ! empty( $invited_students ) ) {
				...
				$content .= '<h2>'. esc_html__( 'Invited Students', 'CP_TD' ) . '</h2>';
			...
			$content .= '<h2>' . esc_html__( 'Invite Student', 'CP_TD' ) .'</h2>';
		...
		$content .= '<h2>' . esc_html__( 'Send an email to enrolled students', 'CP_TD' ) .'</h2>';
		if ( count( $list_course->items ) ) {
			...
			$content .= sprintf( '<th class="row">%s</th>', __( 'Send to', 'CP_TD' ) );
			$content .= '<td><select name="send_to" class="widefat">';
			$send_options = array(
				'all' => __( 'All students', 'CP_TD' ),
				'all_with_submission' => __( 'All students with submissions', 'CP_TD' ),
			);
			$units = CoursePress_Data_Course::get_units( $course_id );

			if ( ! empty( $units ) ) {
				foreach ( $units as $unit ) {
					$send_options[ $unit->ID ] = __( sprintf( 'All students with submissions of &rarr; %s', $unit->post_title ), 'CP_TD' );
				}
			}
			...
			$content .= '<th scope="row"><label class="coursepress-email-subject"><span>' . esc_html__( 'Email subject', 'CP_TD' ) . '</span></label></th>';
			$content .= sprintf(
				'<td><input type="text" name="email-subject" class="large-text" id="coursepress-email-subject" /></td>',
				__( 'Enter email subject', 'CP_TD' )
			);
			$content .= '</tr>';
			...
			$content .= '<th scope="row"><label class="coursepress-email-body"><span>' . esc_html__( 'Email body', 'CP_TD' ) . '</span></label></th>';
			$content .= '<td>';
			$content .= sprintf(
				'<p class="description">%s</p>',
				__( 'These codes will be replaced with actual data: STUDENT_FIRST_NAME, STUDENT_LAST_NAME, STUDENT_LOGIN, BLOG_NAME, WEBSITE_NAME, WEBSITE_ADDRESS, LOGIN_ADDRESS, COURSE_ADDRESS, COURSE_EXCERPT, COURSE_NAME, COURSE_OVERVIEW, COURSES_ADDRESS', 'CP_TD' )
			);
			...
			$content .= sprintf(
				...
				esc_attr( $nonce ),
				esc_html__( 'Send', 'CP_TD' )
			);

		} else {
			$content .= sprintf(
				'<p>%s</p>',
				__( 'You can not send email, there is no enroled studens in this course.', 'CP_TD' )
			);
		}
		$content .= '</div>';
