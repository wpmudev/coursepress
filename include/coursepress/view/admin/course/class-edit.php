...
class CoursePress_View_Admin_Course_Edit {
	...
	private static function render_setup_step_7() {
		...
		$content .= '<div class="wide class-size">
			<label class="required">' . __( 'Minimum Grade Required', 'CP_TD' ) . '</label> <input type="text" name="meta_minimum_grade_required" value="'. esc_attr__( $minimum_grade ) . '" />
			<p class="description">'. __( 'The minimum grade required to marked course completion and send course certficates.', 'CP_TD' ) . '</p>
			</div>
		';
		...
		$content .= '<div class="wide">'
			. '<br /><h3>' . __( 'Course Certificate', 'CP_TD' ) . '</h3>'
			//. '<a href="'. esc_url( $test_mail_link ) . '" target="_blank" class="button button-default btn-cert" style="float:right;margin-top:-35px;margin-right: 70px;">' . __( 'Test Mail', 'CP_TD' ) . '</a>'
			. '<a href="'. esc_url( $certificate_link ) . '" target="_blank" class="button button-default btn-cert" style="float:right;margin-top:-35px;">' . __( 'Preview', 'CP_TD' ) . '</a>'
			. '<label>
				<input type="checkbox" name="meta_basic_certificate" value="1" '. checked(1, CoursePress_Data_Course::get_setting( $course_id, 'basic_certificate' ), false ) . ' /> '. __( 'Override course certificate.', 'CP_TD' )
			. '</label>'
			. '<p class="description">' . __( 'Use this field to override general course certificate setting.', 'CP_TD' ) . '</p>'
			. '<label for="meta_basic_certificate_layout">' . __( 'Certificate Content', 'CP_TD' ) . '</label>'
