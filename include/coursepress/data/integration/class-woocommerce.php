...
class CoursePress_Data_Integration_WooCommerce {
	...
	public static function init() {
		...
		add_action( 'add_meta_boxes', array( __CLASS__, 'add_post_parent_metaboxe' ) );
		add_action( 'woocommerce_process_product_meta_simple', array( __CLASS__, 'woo_save_post' ), 999 );
		add_action( 'coursepress_general_options_page', array( __CLASS__, 'add_woocommerce_general_option' ) );
		add_action( 'coursepress_update_settings', array( __CLASS__, 'save_woocommerce_general_option' ), 10, 2 );
	...
	public static function add_post_parent_metaboxe() {
		add_meta_box( 'cp_woo_post_parent', __( 'Parent Course', 'cp' ), array( __CLASS__, 'cp_woo_post_parent' ), 'product', 'side', 'default' );
	}
	...
	public static function cp_woo_post_parent() {
		global $post;
		if ( isset( $post->ID ) ) {
?>
...
<?php
	...
	public static function add_woocommerce_general_option() {
?>
                <div class="postbox">
...
<?php
	...
	public static function save_woocommerce_general_option( $tab, $post ) {
		if ( $tab == 'general' ) {
